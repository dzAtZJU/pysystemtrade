...
from paper.sysbrokers.ccxt.client.ccxt_price_client import tickerWithBS, ccxtPriceClient
from paper.sysbrokers.broker_perpetuals_price_data import brokerPerpetualsPriceData
...
class ibTickerObject(tickerObject):
    def __init__(self, ticker_with_BS: tickerWithBS, broker_client: ccxtPriceClient):
...
class ccxtPerpetualsPriceData(brokerPerpetualsPriceData):
    """
    ...
    @property
    def ib_client(self) -> ccxtPriceClient:
        client = getattr(self, "_ib_client", None)
        if client is None:
            client = self._ib_client = ccxtPriceClient(
    ...
    def get_prices_at_frequency(self, instrument_code, frequency: Frequency,
    ...
    def _get_prices_at_frequency_for_contract_object_no_checking(self,
            instrument_code, frequency: Frequency) -> futuresContractPrices:

        return self._get_prices_at_frequency_for_contract_object_no_checking_with_expiry_flag(instrument_code,
                                                                                              frequency=frequency,
                                                                                              allow_expired=False)

    def _get_prices_at_frequency_for_contract_object_no_checking_with_expiry_flag(self, instrument_code,
                                                                                  frequency: Frequency,
    ...
    def _get_prices_at_frequency_for_ibcontract_object_no_checking(
        ...
        try:
            price_data = self.ib_client.broker_get_historical_perpetuals_data(
                instrument_code,
                bar_freq=freq,
                allow_expired=allow_expired,
            )
