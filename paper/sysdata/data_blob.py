...
class dataBlob(object):
    ...
    def close(self):
        ...
        #     self.ccxt_conn.close_connection()
        #     self.db_ib_broker_client_id.release_clientid(self.ccxt_conn.client_id())

        # No need to explicitly close Mongo connections; handled by Python garbage collection

    @property
    def ccxt_conn(self) -> connectionCCXT:
        ib_conn = getattr(self, "_ccxt_conn", arg_not_supplied)
        if ib_conn is arg_not_supplied:
            ib_conn = self._get_new_ccxt_connection()
            self._ib_conn = ib_conn

        return ib_conn

    def _get_new_ccxt_connection(self) -> connectionCCXT:
        k1, k2, password = get_broker_account()
        return connectionCCXT(k1, k2, password)
