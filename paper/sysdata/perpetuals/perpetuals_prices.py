...
class perpetualsPriceData(baseData):
    """
    ...
    def has_price_data_for_at_frequency(self, instrument_code, frequency: Frequency) -> bool:
    ...
    def write_merged_prices_for_contract_object(
        ...
        self._write_merged_prices_for_contract_object_no_checking(
            instrument_code, futures_price_data
        )

    def write_prices_at_frequency(
        self,
        instrument_code,
        futures_price_data: futuresContractPrices,
        frequency: Frequency,
        ...
        # not_ignoring_duplication = not ignore_duplication
        # if not_ignoring_duplication:
        #     if self.has_price_data_for_contract_at_frequency(instrument_code=instrument_code,
        #                                                      frequency=frequency):
        #         log = futures_contract_object.log(self.log)
        #         log.warn(
        #             "There is already existing data for %s"
    ...
    def update_prices_at_frequency_for_contract(
        ...
        if frequency is MIXED_FREQ:
            assert False
            old_prices = self.get_merged_prices_for_contract_object(instrument_code)
        else:
            old_prices = self.get_prices_at_frequency(instrument_code, frequency=frequency)
        ...
        if frequency is MIXED_FREQ:
            ...
            self.write_prices_at_frequency(
