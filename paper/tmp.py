...
def _tmp(since, symbol, exhcnage):
    if isinstance(since, str):
        since = int(isoparse(since).timestamp() * 1000)
    candles = exhcnage.fetch_ohlcv(symbol, timeframe='1h', since=since)
    return pd.DataFrame(candles)

def tmp(since:str, symbol, exhcnage):
    '''
    '2021-01-01T10:00+08'
    'BTC-USDT-SWAP'
    '''
    ...
    while True:
        ...
        if len(df) == 0 or df.datetime.iloc[-1] == since:
            break
        since = int(df.datetime.iloc[-1])
        
        df.index = pd.to_datetime(df.iloc[:, 0], unit='ms', utc=True).dt.tz_convert('Asia/Hong_Kong').dt.tz_localize(None)
        df.drop(columns=['datetime', 'open', 'high', 'low', 'volume'], inplace=True)
        dfs.append(df)
        
    rdf = pd.concat(dfs, axis=0)
    return rdf[~rdf.index.duplicated()]
